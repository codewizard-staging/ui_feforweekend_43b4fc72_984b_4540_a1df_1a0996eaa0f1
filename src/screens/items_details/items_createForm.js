...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, prodImages, itemsId, numfields;
        const mapItems = MapItems;

        let items = row['items'];

        numfields = Helper.GetAllNumberFields(items);
        if (numfields.length > 0) Helper.UpdateNumberFields(items, numfields);


        // Add Or Update Items
        rslt = await Support.AddOrUpdateItems(items, []);
        if (rslt.status) {
            itemsId = rslt.id;
        } else { return; }

        for (let i = 0; i < mapItems.length; i++) {
            // Add or Update the Items and navigation entity if it is deos not exist
            let navItem = items.find(x => x.uicomponent === mapItems[i].uicomponent);
            if (!Helper.IsJSONEmpty(navItem) && Helper.IsNullValue(navItem.value)) {
                ...
                if (rslt.status) {
                    data = [
                        { key: "ItemId", value: parseInt(itemsId) },
                        { key: navItem.key, value: parseInt(rslt.id) }
                    ];
                    rslt = await Support.AddOrUpdateItems(data);
        ...
        global.AlertPopup("success", "Items is created successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
