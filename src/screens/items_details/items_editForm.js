...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, changes = [], itemsId, items, numfields;

        items = row['items'];
        itemsId = row['items'].find((x) => x.type === 'keyid').value;
                ...
                if (changes.length > 0) {
                    ...
                    if (tmp.length > 0) {
                        ...
                        if (rslt.status) {
                            ...
                            if (Helper.IsNullValue(mapItem.navpropname)) itemsId = rslt.id;
                            ...
                            rslt = await Support.AddOrUpdateItems(data, dropDownOptions);
        ...
        global.AlertPopup("success", "Items is updated successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
