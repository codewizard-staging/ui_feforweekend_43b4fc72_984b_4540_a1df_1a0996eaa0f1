import ItemsJsonConfig from "config/items_product_details_detail_config.json";
...
const MapItems = [
    { navpropname: "", uicomponent: "items", expand: "", exclude: [], func: Support.AddOrUpdateItems }
]

const FetchItemsInfo = async () => {
    let item = {};
    return new Promise(async (resolve) => {
        const keyItems = Object.keys(ItemsJsonConfig);
        keyItems.forEach(elm => {
            let items = [];
            for (let prop of ItemsJsonConfig[elm]) {
...
const FetchItemsDetails = async (itemsId, enums) => {
    ...
    return new Promise(async (resolve) => {
        ...
        if (itemsId) {
            ...
            if ($expand.length > 0) {
                rslt = await Api.GetItemsSingle(itemsId, `$expand=${$expand}`);
            } else {
                rslt = await Api.GetItemsSingle(itemsId);
            }
            
            if (rslt.status) {

                const items = rslt.values;
                ...
                for (let i = 0; i < MapItems.length; i++) {
                    ...
                    const sourceObj = Helper.IsNullValue(source) ? items : items[source];
...
const Extract = async (itemsId) => {

    return new Promise(async (resolve) => {

        let rtnObj = { row: {}, options: [], backRow: {} };

        await FetchItemsInfo().then(async (item) => {
            ...
            await FetchDropdownItems(items).then(async (enums) => {
                ...
                if (!Helper.IsNullValue(itemsId)) {
                    await FetchItemsDetails(itemsId, enums).then(({ row, backRow }) => {
