...
const columns = [
    ...
    { headerName: "ItemName", field: "ItemName", flex: 1 },
    { headerName: "ItemDescription", field: "ItemDescription", flex: 1 },
    { headerName: "ItemImage", field: "ItemImage", flex: 1 },
    { headerName: "ItemType", field: "ItemType", flex: 1 },
];
...
const Component = (props) => {
    ...
    const OnCloseClicked = async (e) => {
        if (e) {
            const rslt = await Api.SetItemsSingle({ ItemId: deletedId, Deleted: true });
            if (rslt.status) {
                setInitialize(true);
    ...
    const OnActionClicked = (id, type) => {
        let _route;
        if (type === 'edit') _route = `/Items/edit/${id}`;
        if (type === 'view') _route = `/Items/view/${id}`;
    ...
    return (
        ...
        <>
            <Container {...props}>
                <Box style={{ width: '100%', paddingBottom: 5 }}>
                    ...
                    <Stack direction="row">
                        <Grid container sx={{ justifyContent: 'flex-end' }}>
                                ...
                                onClick={() => NavigateTo("/Items/create")}
                            >
                                <AddBoxIcon />
                            </IconButton>
                        </Grid>
                ...
                <Box style={{ width: '100%' }}>
                    <DataTable keyId={'ItemId'} columns={columns} rowsCount={rowsCount} rows={rows} sortBy={sortBy} pageInfo={pageInfo} onActionClicked={OnActionClicked}
